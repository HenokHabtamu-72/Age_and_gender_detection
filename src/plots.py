...
from config import AGE_BUCKET_LABELS
...
def plot_gender_distribution(df: pd.DataFrame, path: Path) -> None:
    counts = df["gender"].map({0: "male", 1: "female"}).value_counts().sort_index()
...
def plot_split_age_boxplot(df: pd.DataFrame, path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    data = [df.loc[df["split"] == split_name, "age"].to_numpy() for split_name in order]
...
def plot_history(history_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    plot_specs = [
        ...
        ("Validation age MAE", "training_age_mae.png", [("train_age_mae", "train_age_mae"), ("val_age_mae", "val_age_mae")], "mae"),
        ("Validation gender accuracy", "training_gender_accuracy.png", [("train_gender_accuracy", "train_gender_accuracy"), ("val_gender_accuracy", "val_gender_accuracy")], "accuracy"),
    ]
    if "train_age_bucket_accuracy" in history_df.columns and "val_age_bucket_accuracy" in history_df.columns:
        plot_specs.append(
            ("Age bucket accuracy", "training_age_bucket_accuracy.png", [("train_age_bucket_accuracy", "train_age_bucket_accuracy"), ("val_age_bucket_accuracy", "val_age_bucket_accuracy")], "accuracy")
        )
...
def plot_bucket_confusion(cm: np.ndarray, path: Path) -> None:
    fig, ax = plt.subplots(figsize=(8, 6))
    disp = ConfusionMatrixDisplay(cm, display_labels=AGE_BUCKET_LABELS)
    disp.plot(ax=ax, xticks_rotation=45, colorbar=False)
    ax.set_title("Age bucket confusion matrix")
    _apply_grid(ax)
    save_plot(fig, path)
...
def plot_group_metrics(group_df: pd.DataFrame, path: Path) -> None:
    fig, ax = plt.subplots(figsize=(9, 5))
    ax.bar(group_df["group"], group_df["age_mae"])
    ax.set_title("Age MAE by subgroup")
    ax.set_xlabel("Group")
    ax.set_ylabel("MAE")
...
def plot_benchmark_bars(benchmark_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for metric, fname, title in [
        ...
        ("age_bucket_accuracy", "benchmark_bar_age_bucket_accuracy.png", "Benchmark comparison: age bucket accuracy (higher is better)"),
    ]:
